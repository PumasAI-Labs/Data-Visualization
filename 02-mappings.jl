...
# Coloring
plt_colors = data(demographics_df) *
                mapping(
                    :AGE,
                    :WEIGHT;
...
# Markers
plt_markers = data(demographics_df) *
                mapping(
                    :AGE,
                    :WEIGHT;
                    marker=:ISMALE => nonnumeric
                ) *
                visual(Scatter)
draw(plt_markers)

## Tip: you can combine different arguments in a single plot
plt_combined = data(demographics_df) *
                mapping(
                    :AGE,
                    :WEIGHT;
                    color=:ISMALE => nonnumeric,
...
# Faceting
## Columns
plt_cols = data(demographics_df) *
            mapping(
                :AGE,
                :WEIGHT;
                col=:ISMALE => nonnumeric
            ) *
            visual(Scatter)
draw(plt_cols)

## Rows
plt_rows = data(demographics_df) *
            mapping(
                :AGE,
                :WEIGHT;
                row=:ISMALE => nonnumeric
            ) *
            visual(Scatter)
draw(plt_rows)
...
plt_subjects = data(df_iv) * # Concentration profiles for each subject
            mapping(
                :time,
                :conc;
