...
data(demographics_df) * mapping(:AGE, :WEIGHT) * visual(Scatter) |> draw # The axis labels are the same as the column names
...
draw(plt_iv; palettes = (; color = [:navyblue, :limegreen, :coral]))
...
data(demographics_df) *
mapping(:ISMALE => nonnumeric, :AGE) *
visual(BoxPlot; color = :firebrick, show_notch = true, whiskerwidth = 0.2) |> draw
...
draw(plt_rename; legend = (; position = :top, markersize = 15, titleposition = :left))
