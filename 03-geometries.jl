...
data(demographics_df) * mapping(:AGE) * histogram() |> draw
...
data(iv_summary) *
mapping(:time, :conc_mean; color = :dose => nonnumeric) *
...
data(iv_summary) *
mapping(:time, :conc_mean, :conc_std; color = :dose => nonnumeric) *
(visual(Errorbars) + visual(ScatterLines)) |> draw # We go over this in the next script

# Box plots
data(demographics_df) * mapping(:ISMALE => nonnumeric, :AGE) * visual(BoxPlot) |> draw

# Violin plots
data(demographics_df) * mapping(:ISMALE => nonnumeric, :AGE) * visual(Violin) |> draw
